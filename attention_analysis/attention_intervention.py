...
# %% Build probe direction matrix D [d_model, n_dirs]
dirs = []
...
D = t.nan_to_num(t.stack(dirs, dim=1).reshape(dirs[0].shape[0], -1))  # [d_model, 192]
...
# %% Sweep over layer configurations
tag = "keep_proj"
layers_chosen = [
    (0,),
    (0, 1, 2, 3, 4, 5, 6, 7),
...
for layers in layers_chosen:
    logits_patch  = intervention_Direction(D,        layers, tag)
...
# %% Results table
table = Table(
    title=f"V-ablation: {tag}   (clean accuracy: {clean_accuracy[-1]*100:.2f}%)",
    show_lines=True,
)
...
Console(record=True).print(table)
